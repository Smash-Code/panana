...
const ImageSlider = ({ images: propImages, selectedIndex, onImageClick }) => {
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(diffX) > swipeThreshold) {
      if (diffX > 0) {
        ...
        onImageClick((selectedIndex + 1));
      } else {
        // Swipe right - go to previous
        onImageClick(
          (selectedIndex - 1 + NUM_DISPLAY_ITEMS) % NUM_DISPLAY_ITEMS,
        );
      }
    }
  ...
  return (
    <div
      className="flex lg:pt-[50px] items-center justify-center w-full h-full"
      style={{ perspective: "1800px", overflow: "hidden" }}
    >
      <div
        ref={sliderRef}
        className="relative"
        ...
        {displayImages.map((image, index) => (
          ...
          >
            <Image
              ...
              priority={isCenter(index, selectedIndex)}
              style={{
